...
static void send_custom_app_data(unsigned char *data, unsigned int len) {
	...
	if(spi_host_model!=UNKNOWN)
	{
		...
		for(int i=0; i<num_of_id; i++)
		{
			...
			if(id_set[i]==0)
			{
				switch(comm_set[i]) {
					...
					case COMM_SET_SERVO:
						...
						spi_comm_set_index[i] = COMM_SET_SERVO;
						break;
					case COMM_SET_FINDHOME:
						value_set[i] = (float)buffer_get_int32(data, &ind) / 1000.0;
						app_huboq_find_home(value_set[i]);
						spi_comm_set_index[i] = COMM_SET_FINDHOME;
						break;
					case COMM_SET_DUTY_PAIR:
						value_set[i] = (float)buffer_get_int32(data, &ind);
						app_huboq_duty_pair((int)value_set[i]);
						spi_comm_set_index[i] = COMM_SET_DUTY_PAIR;
						break;
					case COMM_ALIVE:
						ind += 4;
						spi_comm_set_index[i] = COMM_ALIVE;
						timeout_reset();
						//cdi
						app_vescuino_set_dps_timeout_reset(); // dps control timeout reset
						break;
					default:
					    ind += 4;
			...
			else
			{
				...
				//if(spi_devel_print==1) debug_printf("can dev id=%d, msg=%d\r\n", id_set[i], msg);
				...
				switch(comm_set[i]) {
					case COMM_SET_DUTY:
						...
						spi_comm_set_index[i] = COMM_SET_DUTY;
						//if(msg!=0) 
						comm_can_set_duty(id_set[i], value_set[i]);
						break;
					case COMM_SET_CURRENT:
						value_set[i] = (float)buffer_get_int32(data, &ind) / 1000.0;
						spi_comm_set_index[i] = COMM_SET_CURRENT;
						//if(msg!=0) 
						comm_can_set_current(id_set[i], value_set[i]);
						break;
					case COMM_SET_CURRENT_BRAKE:
						value_set[i] = (float)buffer_get_int32(data, &ind) / 1000.0;
						spi_comm_set_index[i] = COMM_SET_CURRENT_BRAKE;
						//if(msg!=0) 
						comm_can_set_current_brake(id_set[i], value_set[i]);
						break;
					case COMM_SET_RPM:
						value_set[i] = (float)buffer_get_int32(data, &ind);
						spi_comm_set_index[i] = COMM_SET_RPM;
						//if(msg!=0) 
						comm_can_set_rpm(id_set[i], value_set[i]);
						break;
					case COMM_SET_POS:
						value_set[i] = (float)buffer_get_int32(data, &ind) / 1000000.0;
						spi_comm_set_index[i] = COMM_SET_POS;
						//if(msg!=0) 
						comm_can_set_pos(id_set[i], value_set[i]);
						break;
					case COMM_SET_DPS:
						value_set[i] = (float)buffer_get_int32(data, &ind) / 1000.0;
						spi_comm_set_index[i] = COMM_SET_DPS;
						//if(msg!=0) 
						comm_can_set_dps(id_set[i], value_set[i]);
						break;
					case COMM_SET_SERVO:
						value_set[i] = (float)buffer_get_int32(data, &ind) / 1000.0 ;
						spi_comm_set_index[i] = COMM_SET_SERVO;
						//if(msg!=0) 
						comm_can_set_goto(id_set[i], value_set[i]);
						break;
					case COMM_SET_FINDHOME:
						value_set[i] = (float)buffer_get_int32(data, &ind) / 1000.0;
						spi_comm_set_index[i] = COMM_SET_FINDHOME;
						//if(msg!=0) 
						comm_can_set_findhome(id_set[i], value_set[i]);
						break;
					case COMM_SET_DUTY_PAIR:
						value_set[i] = (float)buffer_get_int32(data, &ind);
						spi_comm_set_index[i] = COMM_SET_DUTY_PAIR;
						//if(msg!=0) 
						comm_can_set_dutypair(id_set[i], value_set[i]);
						break;
					case COMM_ALIVE:
						ind += 4;
						spi_comm_set_index[i] = COMM_ALIVE;
						comm_can_set_alive(id_set[i]);
						break;
					default:
						ind += 4;
						spi_comm_set_index[i] = -1;	//error
			...
			if(spi_debug_print==1) debug_printf("custom set! i=%d, ind=%d, id=%d, comm_set=%d, value=%.4f\r\n", i, ind, id_set[i], comm_set[i], (double)value_set[i]);
		}
		
		// raw data print
		if(spi_debug_print==1) 
		{
			debug_printf("custom set raw data: ");
			for(unsigned int i=0; i<len; i++) debug_printf("%d ", data[i]);
			debug_printf("\r\n");
		}
	}
	...
	//buffer_append_float16(send_buffer, mc_interface_get_tot_current(), 1e2, &ind);
	//buffer_append_float16(send_buffer, mc_interface_get_duty_cycle_now(), 1e3, &ind);
	//buffer_append_int32(send_buffer, app_get_custum_status_code(), &ind);
	...
	for(int i=0; i<can_devs_num; i++)
	{
		...
		if(can_st_msg!=0) // +11 byte
		{
			...
			*/
			...
			// 191201
			buffer_append_float32(send_buffer, can_st_msg->rps, 1e5, &ind);
			buffer_append_float16(send_buffer, can_st_msg->current, 1e2, &ind);		// original can_status_msg
			buffer_append_float16(send_buffer, can_st_msg->duty, 1e3, &ind);		// original can_status_msg	
...
static THD_FUNCTION(custom_cmd_debug_thread, arg) {
	...
	for(;;) {
		...
		{
			debug_printf("\r\n--------------------------------------------------------");
			debug_printf("\r\n 0 - enc_rps:%f, current:%f, duty:%f, custom_status:%d", (double)encoder_read_rps(), (double)mc_interface_get_tot_current(), (double)mc_interface_get_duty_cycle_now(), app_huboq_get_status_code());
			for(int i=1; i<VESCUINO_STACK_NUM_MAX; i++)
			{
				if(active_can_devs[i]==1)
				{
					can_st_msg = comm_can_get_status_msg_id(i);
					debug_printf("\r\n %d - enc_rps:%f, current:%f, duty:%f", i, (double)can_st_msg->rps, (double)can_st_msg->current, (double)can_st_msg->duty);
				}
			}
		}
