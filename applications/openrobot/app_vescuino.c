...
static void cmd_set_custom_app_mode(BaseSequentialStream *chp, int argc, char *argv[]) {
	(void)argv;
	if (argc <= 0) {
		chprintf(chp, "Usage: sca 0 <set custom app - 0:VESCuino SPI, 1:SMG, 2:Diff, 3:HuboQ>\r\n");
		return;
	}
	...
	if(mode==0)	{
		...
		appconf->app_custom.custom_app_mode = mode;
		conf_general_store_app_configuration(appconf);
		chprintf(chp, "Set Custom App : Diff Robot\r\n");
	}
	else if(mode==3) {
		appconf->app_custom.custom_app_mode = mode;
		conf_general_store_app_configuration(appconf);
		chprintf(chp, "Set Custom App : HuboQ\r\n");
	}
	else {
...
static THD_FUNCTION(app_vescuino_thread, arg) {
	...
	*/
	...
	for(;;) {
		...
		if(appconf->app_custom.custom_app_mode==CUSTOM_APP_VESCuino || appconf->app_custom.custom_app_mode==CUSTOM_APP_HUBOQ_REMASTERED)	// VESCuino SPI
		{
